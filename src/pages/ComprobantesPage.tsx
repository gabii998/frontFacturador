...
export default function ComprobantesPage(){
  ...
  const filterChips = useMemo(() => ([
    { label: 'Punto de venta', value: pv },
    { label: 'Tipo AFIP', value: tipo },
    { label: 'Últimos', value: limite }
  ]), [pv, tipo, limite])

  return (
    <div className="space-y-6">
      <section className="space-y-4">
        <div className="card bg-gradient-to-r from-blue-50 via-sky-50 to-indigo-50 border border-blue-100">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <button type="button" className="btn" onClick={() => setFiltersOpen(prev => !prev)}>{filtersOpen ? "Ocultar filtros" : "Mostrar filtros"}</button>
          </div>
        </div>

        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div className="card border border-slate-200 bg-white/90 shadow-sm">
            <span className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Importe total</span>
            <p className="text-2xl font-semibold text-slate-800 mt-2">{stats ? formatCurrency.format(stats.totalAmount) : "—"}</p>
            <p className="text-xs text-slate-500 mt-1">Suma de los importes totales recibidos.</p>
          </div>
          <div className="card border border-slate-200 bg-white/90 shadow-sm">
            <span className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Comprobantes listados</span>
            <p className="text-2xl font-semibold text-slate-800 mt-2">{stats ? stats.total : "—"}</p>
            <p className="text-xs text-slate-500 mt-1">Registros recuperados con los filtros actuales.</p>
          </div>
          <div className="card border border-slate-200 bg-white/90 shadow-sm">
            <span className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">Con CAE válido</span>
            <p className="text-2xl font-semibold text-slate-800 mt-2">{stats ? stats.withCae : "—"}</p>
            <p className="text-xs text-slate-500 mt-1">Cantidad de comprobantes autorizados por AFIP.</p>
          </div>
        </div>
      </section>
      ...
      <section className="card space-y-4">
        ...
        <div className="grid md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Máximo de registros a recuperar.</p>
          </div>
